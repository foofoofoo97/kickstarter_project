...
describe("campaigns", () => {
  ...
  it("allow people to contribute money and marks them as approvers", async () => {
    const balanceBefore = await web3.eth.getBalance(campaign.options.address);
    await campaign.methods
      .contribute()
      .send({ value: "200", from: accounts[1] });

    const balanceAfter = await web3.eth.getBalance(campaign.options.address);
    console.log(balanceAfter - balanceBefore);
    assert.equal(200, balanceAfter - balanceBefore);
